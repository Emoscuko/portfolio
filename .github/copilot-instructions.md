...
# Autonomous Agent Review Guidelines

Dear GitHub Copilot (Reviewer),
You are reviewing a Pull Request generated by an autonomous coding agent (Antigravity). Please evaluate the code based on the following strict criteria:

## 1. Security & Safety
* Reject any hardcoded secrets, API keys, or passwords.
* Ensure no destructive commands (like `rm -rf`) or unsafe eval statements are introduced.

## 2. Code Quality & Cleanliness
* Code must be concise and avoid unnecessary complexity.
* Ensure there are no leftover debug print statements or `console.log` unless strictly required by the feature.
* Variables and functions must be descriptively named.

## 3. Scope of Work
* The PR must ONLY address the specific TODO mentioned in the PR description.
* Reject the PR if the agent refactored unrelated files or changed things outside the scope of the request.

## 4. How to Provide Feedback
* If the code violates any of these rules, request changes and provide a specific explanation of what lines need fixing.
* If the code perfectly meets the requirements and is safe to merge, simply approve the PR.
